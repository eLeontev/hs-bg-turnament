...
export const useMinionHitpointsStyles = createStyles<
    string,
    { countOfHitpoints: number; isTriple: boolean }
>((theme: MantineTheme, { countOfHitpoints, isTriple }) => ({
    countOfHitpointsContainer: {
        position: 'absolute',
        width: 50,
        height: 50,
        left: isTriple ? 146 : 149,
        top: isTriple ? 231 : 230,
...
export const useMinionAttackPowerStyles = createStyles<
    string,
    { attackPower: number; isTriple: boolean }
>((theme: MantineTheme, { attackPower, isTriple }) => ({
